...
const ChessBoard = () => {
  ...
  const movePiece = (toRow, toCol) => {
    ...
    // Pawn promotion
    if (movingPiece.slice(1).toLowerCase() === 'pawn' && (toRow === 0 || toRow === 7)) {
      console.log('Im triggering a pawn promotion')
      newBoard[fromRow][fromCol] = '';
      setBoard(newBoard);
      setSelectedPiece(null);
      setPromotion({ row: toRow, col: toCol, color: movingPiece[0] });
      return; // Stop further updates until promotion is handled.
    }
  ...
  const PromotionModal = ({ color, onPromote }) => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
      <div className="bg-white p-4 rounded shadow-lg">
        <h3 className="text-center mb-4">Promote Pawn</h3>
